...
def main():
    ...
    print(f'Hidden: {config.hidden_size1}, {config.hidden_size2}')
    ...
    input(f'input/output dim: {X_train.shape[-1]}/{y_train.shape[-1]}')

    agent = Agent(X_train.shape[-1], config.hidden_size1, config.hidden_size2, y_train.shape[-1], device=config.device)
